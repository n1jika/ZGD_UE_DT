...
void UTargetSelectionWidget::BuildWidgetTreeIfNeeded()
{
	...
	HeightValueText->SetText(FText::FromString(TEXT("Target Height£º0 cm")));
...
void UTargetSelectionWidget::SetHeightValue(float InHeightCm)
{
	if (HeightValueText)
	{
		HeightValueText->SetText(FText::FromString(FString::Printf(TEXT("Target Height£º%.0f cm"), InHeightCm)));
	}
}
